...
public class HttpActivity extends AppCompatActivity {
    ...
    public void handleBtnGet(View v) throws IOException {
                String url = "http://192.168.3.138:8080/IoTWebProgramming/http/exam03";
    ...
    public void handleBtnMultipart(View v) {
        //순수 파일이름을 보내주기 위한 파일 이름 얻는 작업
        String filePath = txtAttach.getText().toString();
        String[] paths = filePath.split("/");
        String fileName = paths[paths.length-1];
        Log.i(TAG, fileName);

        String url = "http://192.168.3.138:8080/IoTWebProgramming/http/exam04";
    ...
    public void handleBtnWebSocketOpen(View v) {
        String url = "ws://192.168.3.138:8080/SensingCarRemoteWebControl/websocket/thermistorsensor";
        Request request = new Request.Builder()
                .url(url)
                .build();
        OkHttpClient client = new OkHttpClient();
    ...
    public void handleBtnChangeAngle(View v) {
        String url = "http://192.168.3.138:8080/SensingCarRemoteWebControl/ultrasonicsensor";
        HttpUrl httpUrl = HttpUrl.parse(url).newBuilder()
